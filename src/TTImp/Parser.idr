...
mutual
  ...
  pibindAll p ((n, ty) :: rest) scope
           = IPi p Nothing n ty (pibindAll p rest scope)
  ...
  explicitLam fname indents
    ...
    where
      ...
      bindAll ((n, ty) :: rest) scope
         = ILam Explicit Nothing (Just n) ty (bindAll rest scope)
  ...
  implicitLam fname indents
     ...
     where
       ...
       bindAll [] scope = scope
       bindAll ((n, ty) :: rest) scope
           = ILam Implicit Nothing (Just n) ty (bindAll rest scope)
  ...
  pat fname indents
     ...
     where
       ...
       bindAll ((p, n, ty) :: rest) scope
           = IPatvar Nothing n ty (bindAll rest scope)
